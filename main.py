...
示例:
  ...
  python main.py --version            # 显示版本号
    ...
    parser.add_argument(
        '--mode', '-m',
        type=str,
        choices=['select', 'optimize', 'signal'],
        help='运行模式 (覆盖配置文件设置)'
    )
    ...
    parser.add_argument(
        ...
        type=str,
        metavar='DATE',
        help='(已废弃) Walk-Forward 分析的当前日期 T'
    )
...
def main():
    ...
    if risk_cfg.get('check_trading_time', False):
        from utils.utils import check_trading_time
